classdef TwoLayerNetwork < Network
    %TWOLAYERNETWORK incapsulate the TwoLayerGraph class.
    ...
    properties (Constant)
        ...
        INTRA_DEMAND_MIN0 = 5;
        DOWN_DEMAND_MAX = TwoLayerNetwork.UP_DOWN_RATIO*TwoLayerNetwork.UP_DEMAND_MAX;
        DOWN_DEMAND_MIN = TwoLayerNetwork.UP_DOWN_RATIO*TwoLayerNetwork.UP_DEMAND_MIN;
    end
    properties
        index;      %{Router,GBS,NBS}
        node;       %{#}
        link;       %{#, capacity, hop}
    ...
    methods
        %% Constructor
        %   TwoLayerNetwork(layer_size, graph_seed, traffic_seed, link_capacity, inter_flow_density, intra_flow_density)
        %   TwoLayerNetwork(model)
        %
        ...
        function this = TwoLayerNetwork(arg1, graph_seed, traffic_seed, ...
            ...
            if ischar(arg1)
                ...
                if nargin>=8 && isempty(link_capacity)==0
                    this.graph = TwoLayerNetwork.Build(layer_size(1), ...
                        cluster_size(2), graph_seed, link_capacity);
                else %not specify the link capacity
                    this.graph = TwoLayerNetwork.Build(cluster_number, ...
